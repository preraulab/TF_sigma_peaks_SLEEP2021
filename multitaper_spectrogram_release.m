function [mt_spectrogram,stimes,sfreqs] = multitaper_spectrogram_release(varargin)
...
mt_spectrogram = zeros(num_windows,sum(freq_inds));
...
parfor n = 1:num_windows
    ...
    %Take the mean FFT magnitude (STEP 4)
...
% window_idxs = window_start' + repmat((0:datawin_size-1), length(window_start), 1); % for previous MATLAB versions
end
