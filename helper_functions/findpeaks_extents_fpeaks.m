%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a version of MATLAB findpeaks.m modified to work with linearized
% input and mod division of X indices to prevent rounding errors of large
% indices. It is used only within this find_frequency_peaks.m context, and
% it produces identical numerical results as findpeaks.m on shorter
...
[y,yIsRow,x,xIsRow,minH,minP,minW,maxW,minD,minT,maxN,sortDir,annotate,refW, mod_val] ...
...
% if needWidth
  ...
  % (wxPk)
  [iPk,bPk,bxPk,byPk,wxPk] = findExtents(y,x,iPk,iFinite,iInfinite,iInflect,minP,minW,maxW,refW, mod_val);
% else
...
%--------------------------------------------------------------------------
function [y,yIsRow,x,xIsRow,Ph,Pp,Wmin,Wmax,Pd,Th,NpOut,Str,Ann,Ref, mod_val] = parse_inputs(Yin,varargin)
...
%--------------------------------------------------------------------------
function [iPk,bPk,bxPk,byPk,wxPk] = findExtents(y,x,iPk,iFin,iInf,iInflect,minP,minW,maxW,refW, mod_val)
...
[wxPk,iLBh,iRBh] = getPeakWidth(yFinite,x,iPk,bPk,iLB,iRB,refW, mod_val);
...
%--------------------------------------------------------------------------
function [wxPk,iLBh,iRBh] = getPeakWidth(y,x,iPk,pbPk,iLB,iRB,wRef, mod_val)
if isempty(iPk)
...
wxPk = getHalfMaxBounds(y, x, iPk, base, iLBh, iRBh, mod_val);
end

%--------------------------------------------------------------------------
function bounds = getHalfMaxBounds(y, x, iPk, base, iLB, iRB, mod_val)

% modified by Alex to include a mod division to handle extremely large
% values and cope with rounding errors 
if ~isempty(mod_val)
    x = mod(x, mod_val);
    x(x==0) = mod_val;
end
