...
elseif strcmp(findpeaks_version, 'par') % enable parallel processing 
    ...
    parfor tt = 1:length(time_select)
        ...
        peaks_in_range = peak_freqs >= peak_freq_range(1) & peak_freqs <= peak_freq_range(2);
...
find_freq_range = optionalDefaults{3};
...
find_freq_range(1) = max(sfreqs(1), find_freq_range(1));
find_freq_range(2) = min(sfreqs(end), find_freq_range(2));
assert(sfreqs(1) <= find_freq_range(1) && sfreqs(end) >= find_freq_range(2), 'findpeaks range falls outside the spectrogram frequency range.')
if peak_freq_range(1) < find_freq_range(1)
    find_freq_range(1) = max(sfreqs(1), peak_freq_range(1)-1);
...
if peak_freq_range(2) > find_freq_range(2)
    find_freq_range(2) = min(sfreqs(end), peak_freq_range(2)+1);
...
if verbose
    ...
    disp(['findpeaks frequency range: ', num2str(find_freq_range(1)), 'Hz - ', num2str(find_freq_range(2)), 'Hz'])
